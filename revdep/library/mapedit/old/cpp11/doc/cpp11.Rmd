...
-   In C++, vector indices start at 0, which means that the last element is at position `n - 1`.
    I'll say this again because it's so important: **IN C++, VECTOR INDICES START AT 0**!
...
`cpp11::register` registers a C++ function to be called from R.
...
1.  To practice your function writing skills, convert the following functions into C++.
    ...
    5.  `var()`.
        ...
        Whenever implementing a numerical algorithm, it's always good to check what is already known about the problem.
...
-   We start at `x.begin()` and loop until we get to `x.end()`.
    A small optimization is to store the value of the end iterator so we don't need to look it up each time.
    This only saves about 2 ns per iteration, so it's only important when the calculations in the loop are very simple.
...
Then the easiest way to configure everything is to call `usethis::use_cpp11()`.
Alternatively:
